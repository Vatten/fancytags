...
import dev.vatten.fancytags.commands.Command;
import dev.vatten.fancytags.events.PlayerJoinEvent;
import dev.vatten.fancytags.events.PlayerLeaveEvent;
import dev.vatten.fancytags.events.PlayerLoadInEvent;
...
public class VelocityVattenPlatform implements VattenPlatform<Player, ScheduledTask> {
    ...
    @Override
    public dev.vatten.fancytags.ScheduledTask<ScheduledTask> scheduleTask(Runnable runnable, int delay) {
        return new VelocityScheduledTask(server.getScheduler().buildTask(this, runnable).delay(delay, TimeUnit.MILLISECONDS).schedule());
    }

    @Override
    public dev.vatten.fancytags.ScheduledTask<ScheduledTask> scheduleRepeatingTask(Runnable runnable, int delay, int period) {
